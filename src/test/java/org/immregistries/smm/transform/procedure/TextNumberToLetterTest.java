...
public class TextNumberToLetterTest extends ProcedureCommonTest {

  @Test
  public void test() {
    Transformer transformer = new Transformer();

    assertEquals("A", new TextNumberToLetter(null).varyText("A", transformer));
    assertEquals("a", new TextNumberToLetter(null).varyText("a", transformer));
    assertEquals("0", new TextNumberToLetter(null).varyText("0", transformer));

    assertEquals("1st Son", new TextNumberToLetter(null).varyText("1st Son", transformer));

    assertEquals("Bob", new TextNumberToLetter(null).varyText("Bob", transformer));
    assertEquals("Robert Paulson",
        new TextNumberToLetter(null).varyText("Robert Paulson", transformer));
    assertEquals("Grant-Bob", new TextNumberToLetter(null).varyText("Grant-Bob", transformer));
  ...
  private void testVariationDifferent(String startValue, String location, String procedure,
    ...
    assertNotEquals(startValue, new TextNumberToLetter(null).varyText(startValue, transformer));
  ...
  private void testVariationSame(String startValue, String location, String procedure,
      Transformer transformer) {
    assertEquals(startValue, new TextNumberToLetter(null).varyText(startValue, transformer));
    String testStart = transform(DEFAULT_TEST_MESSAGE, location + "=" + startValue);
